...
# ==================== 거래 내역 ====================
    ...
    with st.expander("➕ 빠른 거래 입력", expanded=False):
        ...
        with col1:
            입력_품목 = st.text_input("품목", key="quick_product", placeholder="품목명 입력")
        with col2:
        ...
        with col4:
            # 자동 계산
            자동_공급가액 = 입력_수량 * 입력_단가
            입력_공급가액 = st.number_input("공급가액", value=자동_공급가액, step=1000, format="%d", key="quick_amount")
...
# ==================== 회계 관리 ====================
    ...
    if len(df) == 0:
        ...
        # 탭 생성
        tab1, tab2, tab3, tab4 = st.tabs(["📊 손익 현황", "🧾 부가세 현황", "💹 마진 분석", "🏢 거래처별 마진"])
        ...
        # ===== 탭2: 부가세 현황 =====
        with tab2:
            st.markdown("### 🧾 부가세 신고 현황")
            
            col1, col2 = st.columns(2)
            with col1:
                선택_연도_부가세 = st.selectbox("연도 선택", 연도_목록, format_func=lambda x: f"{x}년", key="vat_year")
            with col2:
                기간_옵션 = ["1기 (1~6월)", "2기 (7~12월)", "1기 예정 (1~3월)", "1기 확정 (4~6월)", "2기 예정 (7~9월)", "2기 확정 (10~12월)"]
                선택_기간 = st.selectbox("신고 기간", 기간_옵션, key="vat_period")
            ...
            시작월, 종료월 = 기간_월_매핑[선택_기간]
            
            df['연도'] = df['날짜'].dt.year
            df['월'] = df['날짜'].dt.month
            기간_df = df[(df['연도'] == 선택_연도_부가세) & (df['월'] >= 시작월) & (df['월'] <= 종료월)]
            
            # 매출/매입 부가세 계산
            외출_df = 기간_df[기간_df['참조'] == '=외출']
            외입_df = 기간_df[기간_df['참조'] == '=외입']
            
            매출_공급가액 = 외출_df['공급가액'].sum()
            매출_부가세 = 외출_df['부가세'].sum()
            매입_공급가액 = abs(외입_df['공급가액'].sum())
            매입_부가세 = abs(외입_df['부가세'].sum())
            납부세액 = 매출_부가세 - 매입_부가세
            
            # 현금 거래 분석 (입금인데 부가세가 0인 경우 현금 거래로 추정)
            현금_매출 = 기간_df[(기간_df['참조'] == '=외출') & (기간_df['부가세'] == 0)]['공급가액'].sum()
            
            st.markdown(f"#### {선택_연도_부가세}년 {선택_기간}")
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.markdown("##### 📤 매출")
                st.metric("공급가액", f"{매출_공급가액:,.0f}원")
                st.metric("부가세", f"{매출_부가세:,.0f}원")
                st.metric("합계", f"{매출_공급가액 + 매출_부가세:,.0f}원")
                if 현금_매출 > 0:
                    st.info(f"💵 현금매출 추정: {현금_매출:,.0f}원 (부가세 미포함 거래)")
            
            with col2:
                st.markdown("##### 📥 매입")
                st.metric("공급가액", f"{매입_공급가액:,.0f}원")
                st.metric("부가세", f"{매입_부가세:,.0f}원")
                st.metric("합계", f"{매입_공급가액 + 매입_부가세:,.0f}원")
            
            st.markdown("---")
            
            # 납부세액
            if 납부세액 >= 0:
                st.error(f"### 💸 납부할 부가세: {납부세액:,.0f}원")
            else:
                st.success(f"### 💰 환급받을 부가세: {abs(납부세액):,.0f}원")
            
            # 월별 상세
            st.markdown("#### 월별 상세")
            월별_부가세 = []
            for 월 in range(시작월, 종료월 + 1):
                월_df = 기간_df[기간_df['월'] == 월]
                월_매출세 = 월_df[월_df['참조'] == '=외출']['부가세'].sum()
                월_매입세 = abs(월_df[월_df['참조'] == '=외입']['부가세'].sum())
                월별_부가세.append({
                    '월': f"{월}월",
                    '매출부가세': 월_매출세,
                    '매입부가세': 월_매입세,
            ...
            월별_df = pd.DataFrame(월별_부가세)
            display_월별 = 월별_df.copy()
            for col in ['매출부가세', '매입부가세', '차액']:
