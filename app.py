...
menu_list = ["🏠 대시보드", "➕ 거래 입력", "📄 거래 내역", "📊 통계 분석", "💰 외상 관리", "🧾 회계 관리", "📦 품목 관리", "📋 재고 관리", "👥 거래처 관리", "📅 방문 일정", "📝 영업 일지", "🔧 설정"]
...
# ==================== 거래 내역 ====================
    ...
    if len(df) == 0:
        ...
        # 미수금 계산 함수 - base_receivables에서 GULREST 값 사용
        def 거래처별_미수금_계산(거래처명):
            """거래처별 현재 미수금 = 컴장부 GULREST 값"""
            기초미수금_dict = st.session_state.base_receivables_df.set_index('거래처')['기초미수금'].to_dict() if len(st.session_state.base_receivables_df) > 0 else {}
            return 기초미수금_dict.get(거래처명, 0)
...
# ==================== 영업 일지 ====================
    ...
    # ===== 탭2: 잠재거래처 관리 =====
    with tab2:
        st.markdown("### 🎯 잠재거래처 관리")
        st.info("영업 공략 대상 업체를 등록하고 관리하세요. 네이버 지도 등에서 검색한 업체를 수동으로 등록합니다.")
        
        col_reg, col_list = st.columns([1, 1])
        
        with col_reg:
            st.markdown("#### ➕ 잠재거래처 등록")
            ...
            지역_옵션 = ["", "청주시 상당구", "청주시 서원구", "청주시 흥덕구", "청주시 청원구", 
                      "세종시", "대전시", "천안시", "아산시", "음성군", "진천군", "증평군", 
                      "괴산군", "보은군", "옥천군", "영동군", "충주시", "제천시", "단양군",
                      "공주시", "논산시", "계룡시", "금산군", "부여군", "서천군", "청양군", 
                      "홍성군", "예산군", "태안군", "당진시", "전주시", "익산시", "군산시",
                      "수도권", "전남/광주", "경남/부산", "경북/대구", "강원", "기타지역"]
            신규_지역 = st.selectbox("📍 지역", 지역_옵션, key="prospect_region")
            
            # 업종
            업종_옵션 = ["철물점", "건자재점", "농자재점", "공구상", "전기자재", "산업용품", "종합건재", "기타"]
            ...
            with col_c2:
                신규_담당자 = st.text_input("👤 담당자", key="prospect_contact")
            
            신규_주소 = st.text_input("🏠 주소", placeholder="상세 주소", key="prospect_address")
            
            # 영업 단계
            영업단계_옵션2 = ["발굴", "접촉예정", "접촉완료", "상담중", "견적발송", "협상중", "보류", "탈락"]
            신규_영업단계 = st.selectbox("📊 영업 단계", 영업단계_옵션2, key="prospect_stage")
            
            신규_메모 = st.text_area("📝 메모", height=80, placeholder="특이사항, 규모, 가능성 등", key="prospect_memo")
            
            if st.button("💾 잠재거래처 등록", type="primary", use_container_width=True):
                if 신규_업체명:
                    # 중복 체크
                    if 신규_업체명 in 잠재_거래처_list:
                        st.warning(f"'{신규_업체명}'은(는) 이미 등록된 업체입니다.")
                    else:
                        ...
                        st.success(f"✅ '{신규_업체명}' 잠재거래처가 등록되었습니다!")
                        st.rerun()
                else:
                    st.error("❌ 업체명을 입력해주세요.")
        
        with col_list:
            st.markdown("#### 📋 잠재거래처 목록")
            
            if len(prospects_df) > 0:
                # 필터
                지역_필터 = st.text_input("🔍 지역/업체명 검색", placeholder="검색어 입력", key="prospect_search")
                
                if 지역_필터:
                    표시_df = prospects_df[
                        (prospects_df['지역'].str.contains(지역_필터, case=False, na=False)) |
                        (prospects_df['업체명'].str.contains(지역_필터, case=False, na=False)) |
                        (prospects_df['주소'].str.contains(지역_필터, case=False, na=False))
                    ]
                else:
                    표시_df = prospects_df
                
                st.success(f"총 **{len(표시_df)}개** 잠재거래처")
                
                # 영업 단계별 통계
                단계_통계 = 표시_df['영업단계'].value_counts()
                st.markdown("**영업 단계별:**")
                단계_text = " | ".join([f"{k}: {v}개" for k, v in 단계_통계.items()])
                st.caption(단계_text)
                
                # 목록 표시
                for idx, row in 표시_df.iterrows():
                    with st.expander(f"🏢 {row['업체명']} | {row['지역']} | {row['영업단계']}"):
                        st.markdown(f"**업종:** {row['업종']}")
                        st.markdown(f"**전화:** {row['전화번호']}")
                        st.markdown(f"**주소:** {row['주소']}")
                        st.markdown(f"**담당자:** {row['담당자']}")
                        st.markdown(f"**메모:** {row['메모']}")
                        st.caption(f"등록일: {row['등록일']}")
                        
                        # 삭제 버튼
                        if st.button(f"🗑️ 삭제", key=f"del_prospect_{idx}"):
                            prospects_df = prospects_df.drop(idx).reset_index(drop=True)
                            prospects_df.to_csv(prospects_file, index=False, encoding='utf-8-sig')
                            st.success(f"'{row['업체명']}' 삭제됨")
                            st.rerun()
            else:
                st.info("등록된 잠재거래처가 없습니다.")
        
        # 지역별 현황
        if len(prospects_df) > 0:
            st.markdown("---")
            st.markdown("### 🗺️ 지역별 잠재거래처 현황")
            
            지역_통계 = prospects_df['지역'].value_counts().reset_index()
            ...
            with col_t2:
                if len(지역_통계) > 0:
                    import plotly.express as px
                    fig = px.pie(지역_통계, values='업체수', names='지역', title='지역별 분포')
    ...
    with tab3:
        ...
        st.info("""
        네이버 지도 등에서 검색한 업체 정보를 엑셀로 정리한 후 일괄 업로드할 수 있습니다.
        
        **엑셀 양식:**
        | 업체명 | 지역 | 업종 | 전화번호 | 주소 | 담당자 | 메모 |
        """)
        
        # 양식 다운로드
        st.markdown("#### 📥 양식 다운로드")
        양식_df = pd.DataFrame(columns=['업체명', '지역', '업종', '전화번호', '주소', '담당자', '메모'])
        양식_df.loc[0] = ['예시철물', '대전시', '철물점', '042-000-0000', '대전시 서구 예시로 123', '홍길동', '대형 매장']
            ...
            try:
                ...
                if st.button("💾 일괄 등록", type="primary"):
                    ...
                    if '업체명' not in upload_df.columns:
                        ...
                        # 중복 제거
                        기존_업체 = set(prospects_df['업체명'].tolist())
                        신규_df = upload_df[~upload_df['업체명'].isin(기존_업체)]
                        
                        if len(신규_df) > 0:
                            prospects_df = pd.concat([prospects_df, 신규_df[['업체명', '지역', '업종', '전화번호', '주소', '담당자', '영업단계', '메모', '등록일']]], ignore_index=True)
                            prospects_df.to_csv(prospects_file, index=False, encoding='utf-8-sig')
                            st.success(f"✅ {len(신규_df)}개 업체가 등록되었습니다! (중복 제외)")
                            st.rerun()
                        else:
                            st.warning("모든 업체가 이미 등록되어 있습니다.")
            except Exception as e:
        ...
        if len(prospects_df) > 0:
            ...
            st.download_button(
                label=f"📥 잠재거래처 전체 다운로드 ({len(prospects_df)}개)",
                data=excel_data2,
                file_name=f"잠재거래처_{get_kst_now().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
