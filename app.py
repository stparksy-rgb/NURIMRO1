...
def calculate_receivable(ê±°ëž˜ì²˜, df=None):
    """ê±°ëž˜ì²˜ë³„ ë¯¸ìˆ˜ê¸ˆ ì‹¤ì‹œê°„ ê³„ì‚° (ì „ì²´ ë°ì´í„° ê¸°ë°˜)
    
    ì»´ìž¥ë¶€ ë°ì´í„° êµ¬ì¡°:
    - íŒë§¤: ê³µê¸‰ê°€ì•¡ > 0, ë‚´ìš©ì— 'ìž…ê¸ˆ' ì—†ìŒ
    - ìž…ê¸ˆ: ê³µê¸‰ê°€ì•¡ > 0, ë‚´ìš©ì— 'ìž…ê¸ˆ' í¬í•¨
    
    ë¯¸ìˆ˜ê¸ˆ = íŒë§¤ + ë¶€ê°€ì„¸ - ìž…ê¸ˆ
    """
    if df is None:
    ...
    ê±°ëž˜ì²˜_df = df[df['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜]
    ...
    for _, row in ê±°ëž˜ì²˜_df.iterrows():
        í’ˆëª© = str(row.get('í’ˆëª©', '') or '')
        ì°¸ì¡° = str(row.get('ì°¸ì¡°', '') or '')
        ê³µê¸‰ê°€ì•¡ = row.get('ê³µê¸‰ê°€ì•¡', 0) or 0
        ë¶€ê°€ì„¸ = row.get('ë¶€ê°€ì„¸', 0) or 0
        
        # ì–‘ìˆ˜ ê±°ëž˜ë§Œ ì²˜ë¦¬ (íŒë§¤/ìž…ê¸ˆ)
        if ê³µê¸‰ê°€ì•¡ > 0:
            if 'ìž…ê¸ˆ' in í’ˆëª© or 'ìž…ê¸ˆ' in ì°¸ì¡°:
                # ìž…ê¸ˆ
                ì´_ìž…ê¸ˆ += ê³µê¸‰ê°€ì•¡
            else:
                # íŒë§¤
    ...
    # ë¯¸ìˆ˜ê¸ˆ = íŒë§¤ + ë¶€ê°€ì„¸ - ìž…ê¸ˆ
    ë¯¸ìˆ˜ê¸ˆ = ì´_íŒë§¤ + ì´_ë¶€ê°€ì„¸ - ì´_ìž…ê¸ˆ
    
    return ë¯¸ìˆ˜ê¸ˆ

def calculate_payable(ê±°ëž˜ì²˜, df=None):
    """ê±°ëž˜ì²˜ë³„ ë¯¸ì§€ê¸‰ê¸ˆ ê³„ì‚° (ë§¤ìž…ì²˜ì— ì¤„ ëˆ)
    
    ë¯¸ì§€ê¸‰ê¸ˆ = |ë§¤ìž…| + |ë§¤ìž…ë¶€ê°€ì„¸| - |ì¶œê¸ˆ/ê²°ì œ|
    """
    if df is None:
        df = st.session_state.ledger_df
    ...
    for _, row in ê±°ëž˜ì²˜_df.iterrows():
        ...
        # ìŒìˆ˜ ê±°ëž˜ë§Œ ì²˜ë¦¬ (ë§¤ìž…/ì¶œê¸ˆ)
        if ê³µê¸‰ê°€ì•¡ < 0:
            if 'ìž…ê¸ˆ' in í’ˆëª© or 'ì¶œê¸ˆ' in í’ˆëª© or 'ì¶œê¸ˆ' in ì°¸ì¡°:
                # ì¶œê¸ˆ/ê²°ì œ
                ì´_ì¶œê¸ˆ += abs(ê³µê¸‰ê°€ì•¡)
            else:
                # ë§¤ìž…
                ì´_ë§¤ìž… += abs(ê³µê¸‰ê°€ì•¡)
                ì´_ë¶€ê°€ì„¸ += abs(ë¶€ê°€ì„¸)
    
    # ë¯¸ì§€ê¸‰ê¸ˆ = ë§¤ìž… + ë¶€ê°€ì„¸ - ì¶œê¸ˆ
    ë¯¸ì§€ê¸‰ê¸ˆ = ì´_ë§¤ìž… + ì´_ë¶€ê°€ì„¸ - ì´_ì¶œê¸ˆ
    
    return ë¯¸ì§€ê¸‰ê¸ˆ

def calculate_all_receivables(df=None):
    """ì „ì²´ ê±°ëž˜ì²˜ë³„ ë¯¸ìˆ˜ê¸ˆ = base_receivables (ì»´ìž¥ë¶€ GULREST)"""
    ...
    ë¯¸ìˆ˜ê¸ˆ_df = ë¯¸ìˆ˜ê¸ˆ_df.rename(columns={'ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ': 'ë¯¸ìˆ˜ê¸ˆ'})
    ë¯¸ìˆ˜ê¸ˆ_df['ìµœê·¼ê±°ëž˜ì¼'] = ë¯¸ìˆ˜ê¸ˆ_df['ê¸°ì¤€ì¼ìž']
    
    return ë¯¸ìˆ˜ê¸ˆ_df

def calculate_all_payables(df=None):
    """ì „ì²´ ê±°ëž˜ì²˜ë³„ ë¯¸ì§€ê¸‰ê¸ˆ = base_receivables (ì»´ìž¥ë¶€ GULREST, ìŒìˆ˜)"""
    base_recv = st.session_state.base_receivables_df.copy()
    ...
    # ìŒìˆ˜ë§Œ (ë¯¸ì§€ê¸‰ê¸ˆ)
    ë¯¸ì§€ê¸‰ê¸ˆ_df = base_recv[base_recv['ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ'] < 0].copy()
    ë¯¸ì§€ê¸‰ê¸ˆ_df['ë¯¸ì§€ê¸‰ê¸ˆ'] = ë¯¸ì§€ê¸‰ê¸ˆ_df['ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ'].abs()
    ë¯¸ì§€ê¸‰ê¸ˆ_df['ìµœê·¼ê±°ëž˜ì¼'] = ë¯¸ì§€ê¸‰ê¸ˆ_df['ê¸°ì¤€ì¼ìž']
