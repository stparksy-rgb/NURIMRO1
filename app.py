...
def create_invoice_html(거래처, 날짜, 거래_목록):
    ...
    for i, 거래 in enumerate(거래_목록, 1):
        ...
        <tr>
            ...
            <td>{품목}</td>
            <td style="text-align: right;">{수량:,.0f}</td>
            <td style="text-align: right;">{단가:,.0f}</td>
            <td style="text-align: right;">{금액:,.0f}</td>
        </tr>
        """
    ...
    <head>
        ...
        <style>
            ...
            .title {{ text-align: center; font-size: 24px; font-weight: bold; margin: 20px 0 30px 0; letter-spacing: 10px; }}
            .info-table {{ width: 100%; margin-bottom: 20px; }}
            .info-table td {{ padding: 5px; }}
            .info-left {{ text-align: left; }}
            .info-right {{ text-align: right; }}
            table.main {{ width: 100%; border-collapse: collapse; margin: 20px 0; }}
            table.main th, table.main td {{ border: 1px solid #333; padding: 8px; }}
            table.main th {{ background-color: #f0f0f0; }}
            .summary {{ margin-top: 20px; text-align: right; }}
            .summary-line {{ margin: 5px 0; font-size: 14px; }}
    ...
    <body>
        ...
        <table class="info-table">
            <tr>
                <td class="info-left">
                    <strong>거래처:</strong> {거래처}<br>
                    <strong>거래일:</strong> {날짜}
                </td>
                <td class="info-right">
                    <strong>공급자:</strong> 누리엠알오<br>
                    <strong>사업자번호:</strong> 301-03-55081
                </td>
            </tr>
        </table>
        ...
        <div class="summary">
            <div class="summary-line">공급가액: {총공급가액:,.0f}원</div>
            <div class="summary-line">부 가 세: {총부가세:,.0f}원</div>
            <div class="total">합 계: {총공급가액 + 총부가세:,.0f}원</div>
        </div>
