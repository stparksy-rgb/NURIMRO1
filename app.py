    ...
    with col1:
        ...
        if 품목입력방식 == "품목 검색":
            if len(products_df) > 0:
                # 검색 필터 (개선된 자동완성!)
                검색어 = st.text_input("품목 검색", placeholder="1~2글자만 입력하세요 (예: 절, TU)")
                
                # 한글 1~2자 또는 영문 2자 이상 입력 시 자동 검색
                if 검색어 and len(검색어) >= 1:
                    ...
                    if len(검색결과) > 0:
                        ...
                        선택품목 = st.selectbox("✨ 검색 결과 (클릭하여 선택)", [""] + 품목_옵션, key="search_result")
                        ...
                        st.warning("❌ 검색 결과가 없습니다. 다시 검색하거나 '직접 입력'을 선택하세요.")
                        품목 = ""
                else:
                    # 검색어 없으면 안내 메시지
                    st.info("💡 품목명 일부를 입력하세요. 자동으로 관련 품목이 표시됩니다!")
                    품목 = ""
            else:
                st.info("등록된 품목이 없습니다. '직접 입력'을 사용하세요.")
