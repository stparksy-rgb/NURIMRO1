...
# ==================== 협약서 관리 ====================
    ...
    with tab1:
        ...
        st.markdown("---")
        st.markdown("#### ✍️ 전자서명")
        st.warning("아래 서명란에 서명해주세요. (터치 또는 마우스로 서명)")
        
        # 서명 캔버스 (streamlit-drawable-canvas 사용)
        try:
            ...
            with col_sign1:
                st.markdown("**공급자 서명 (박수영)**")
                canvas_supplier = st_canvas(
            ...
            with col_sign2:
                st.markdown(f"**구매자 서명 ({구매자_대표 if 구매자_대표 else ''})**")
                canvas_buyer = st_canvas(
                    fill_color="rgba(255, 255, 255, 0)",
                    stroke_width=3,
                    stroke_color="#000000",
                    background_color="#FFFFFF",
            ...
            st.warning("⚠️ 서명 기능을 사용하려면 streamlit-drawable-canvas 라이브러리가 필요합니다.")
            st.code("pip install streamlit-drawable-canvas")
            
            # 대체 서명 방식 (체크박스)
            col_sign1, col_sign2 = st.columns(2)
            with col_sign1:
                공급자_서명확인 = st.checkbox("✅ 공급자 (박수영) 서명 확인", key="supplier_sign_check")
            with col_sign2:
                구매자_서명확인 = st.checkbox("✅ 구매자 서명 확인", key="buyer_sign_check")
            
            서명_가능 = 공급자_서명확인 and 구매자_서명확인
            canvas_supplier = None
            canvas_buyer = None
