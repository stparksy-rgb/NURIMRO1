    ...
    if len(df) == 0:
        ...
        with tab2:
            ...
            for 월 in range(1, 13):
                ...
                # 분기별 누적
                if 월 <= 3:
                    for k in q1_data: q1_data[k] += row_data.get(k, 0) if k != '구분' and k != 'is_summary' else 0
                elif 월 <= 6:
                    for k in q2_data: q2_data[k] += row_data.get(k, 0) if k != '구분' and k != 'is_summary' else 0
                elif 월 <= 9:
                    for k in q3_data: q3_data[k] += row_data.get(k, 0) if k != '구분' and k != 'is_summary' else 0
                else:
                    for k in q4_data: q4_data[k] += row_data.get(k, 0) if k != '구분' and k != 'is_summary' else 0
            ...
            # 연간 요약 (상단)
            연간_총매출 = q1_data['총매출액'] + q2_data['총매출액'] + q3_data['총매출액'] + q4_data['총매출액']
            ...
            # HTML 테이블로 직접 생성 (하늘색 헤더 + 합계 행 강조)
            html_table = """
            <style>
                .vat-table {
                    width: 100%;
                ...
                .vat-table tr.summary-q td {
                    background-color: #2d4a5e;
                    color: #fff;
                    font-weight: bold;
                }
                .vat-table tr.summary-half td {
                    background-color: #1565c0;
                    color: #fff;
                    font-weight: bold;
                }
                .vat-table tr.summary-year td {
            ...
            for _, row in 월별_df.iterrows():
                구분 = row['구분']
                
                # 행 스타일 결정
                if '연간' in 구분:
                    row_class = 'summary-year'
                elif '반기' in 구분:
                    row_class = 'summary-half'
                elif '분기' in 구분:
                    row_class = 'summary-q'
                else:
                    row_class = ''
            ...
            월_only = 월별_df[월별_df['is_summary'] == False]
            fig = go.Figure()
