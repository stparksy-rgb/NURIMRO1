...
def calculate_receivable(cust, df=None):
    if df is None:
        df = st.session_state.ledger_df
    base_recv = st.session_state.base_receivables_df
    if len(base_recv) == 0:
        return 0
    C0, C1, C2 = base_recv.columns[0], base_recv.columns[1], base_recv.columns[2]
    cb = base_recv[base_recv[C0] == cust]
    if len(cb) == 0:
        return 0
    bv = cb[C1].values[0]
    if bv <= 0:
        return 0
    try:
        bd = pd.to_datetime(str(cb[C2].values[0]))
    except:
        return bv
    if len(df) == 0:
        return bv
    D0, D1, D6, D7, D9 = df.columns[0], df.columns[1], df.columns[6], df.columns[7], df.columns[9]
    cd = df[df[D1] == cust].copy()
    if len(cd) == 0:
        return bv
    cd[D0] = pd.to_datetime(cd[D0])
    cd = cd[cd[D0] > bd]
    if len(cd) == 0:
        return bv
    sm = (cd[D9] == '=\xec\x99\xb8\xec\xb6\x9c'.encode().decode()) | ((cd[D6] > 0) & (~cd[D9].fillna('').str.contains('\xec\x9e\x85\xea\xb8\x88|\xec\xb6\x9c\xea\xb8\x88'.encode().decode())))
    dm = (cd[D9] == '=\xec\x9e\x85\xea\xb8\x88'.encode().decode()) | (cd[D9].fillna('').str.contains('\xec\x9e\x85\xea\xb8\x88'.encode().decode()))
    return bv + cd.loc[sm, D6].sum() + cd.loc[sm, D7].sum() - cd.loc[dm, D6].sum()

def calculate_payable(cust, df=None):
    if df is None:
        df = st.session_state.ledger_df
    base_recv = st.session_state.base_receivables_df
    if len(base_recv) == 0:
    ...
    if len(cd) == 0:
        return bv
    pm = (cd[D9] == '=\xec\x99\xb8\xec\x9e\x85'.encode().decode()) | ((cd[D6] < 0) & (~cd[D9].fillna('').str.contains('\xec\xb6\x9c\xea\xb8\x88|\xec\x9e\x85\xea\xb8\x88'.encode().decode())))
    wm = (cd[D9] == '=\xec\xb6\x9c\xea\xb8\x88'.encode().decode()) | (cd[D9].fillna('').str.contains('\xec\xb6\x9c\xea\xb8\x88'.encode().decode()))
    return bv + abs(cd.loc[pm, D6].sum()) + abs(cd.loc[pm, D7].sum()) - abs(cd.loc[wm, D6].sum())

def calculate_all_receivables(df=None):
    if df is None:
        df = st.session_state.ledger_df
    br = st.session_state.base_receivables_df.copy()
    if len(br) == 0:
        return pd.DataFrame(columns=['\xea\xb1\xb0\xeb\x9e\x98\xec\xb2\x98'.encode().decode(), '\xeb\xaf\xb8\xec\x88\x98\xea\xb8\x88'.encode().decode(), '\xec\xb5\x9c\xea\xb7\xbc\xea\xb1\xb0\xeb\x9e\x98\xec\x9d\xbc'.encode().decode()])
    C0, C1, C2 = br.columns[0], br.columns[1], br.columns[2]
    D0, D1 = (df.columns[0], df.columns[1]) if len(df) > 0 else (C2, C0)
    rc = br[br[C1] > 0][C0].unique()
    rs = []
    for c in rc:
        a = calculate_receivable(c, df)
        if a > 0:
            cd = df[df[D1] == c] if len(df) > 0 else pd.DataFrame()
            ld = pd.to_datetime(cd[D0]).max().strftime('%Y-%m-%d') if len(cd) > 0 else (br[br[C0] == c][C2].values[0] if len(br[br[C0] == c]) > 0 else '')
            rs.append({C0: c, '\xeb\xaf\xb8\xec\x88\x98\xea\xb8\x88'.encode().decode(): a, '\xec\xb5\x9c\xea\xb7\xbc\xea\xb1\xb0\xeb\x9e\x98\xec\x9d\xbc'.encode().decode(): ld})
    if not rs:
        return pd.DataFrame(columns=[C0, '\xeb\xaf\xb8\xec\x88\x98\xea\xb8\x88'.encode().decode(), '\xec\xb5\x9c\xea\xb7\xbc\xea\xb1\xb0\xeb\x9e\x98\xec\x9d\xbc'.encode().decode()])
    return pd.DataFrame(rs).sort_values('\xeb\xaf\xb8\xec\x88\x98\xea\xb8\x88'.encode().decode(), ascending=False)
...
def calculate_all_payables(df=None):
    ...
    br = st.session_state.base_receivables_df.copy()
    if len(br) == 0:
        return pd.DataFrame(columns=['\xea\xb1\xb0\xeb\x9e\x98\xec\xb2\x98'.encode().decode(), '\xeb\xaf\xb8\xec\xa7\x80\xea\xb8\x89\xea\xb8\x88'.encode().decode(), '\xec\xb5\x9c\xea\xb7\xbc\xea\xb1\xb0\xeb\x9e\x98\xec\x9d\xbc'.encode().decode()])
    C0, C1, C2 = br.columns[0], br.columns[1], br.columns[2]
    D0, D1 = (df.columns[0], df.columns[1]) if len(df) > 0 else (C2, C0)
    pc = br[br[C1] < 0][C0].unique()
    rs = []
    for c in pc:
        a = calculate_payable(c, df)
        if a > 0:
            cd = df[df[D1] == c] if len(df) > 0 else pd.DataFrame()
            ld = pd.to_datetime(cd[D0]).max().strftime('%Y-%m-%d') if len(cd) > 0 else (br[br[C0] == c][C2].values[0] if len(br[br[C0] == c]) > 0 else '')
            rs.append({C0: c, '\xeb\xaf\xb8\xec\xa7\x80\xea\xb8\x89\xea\xb8\x88'.encode().decode(): a, '\xec\xb5\x9c\xea\xb7\xbc\xea\xb1\xb0\xeb\x9e\x98\xec\x9d\xbc'.encode().decode(): ld})
    if not rs:
        return pd.DataFrame(columns=[C0, '\xeb\xaf\xb8\xec\xa7\x80\xea\xb8\x89\xea\xb8\x88'.encode().decode(), '\xec\xb5\x9c\xea\xb7\xbc\xea\xb1\xb0\xeb\x9e\x98\xec\x9d\xbc'.encode().decode()])
    return pd.DataFrame(rs).sort_values('\xeb\xaf\xb8\xec\xa7\x80\xea\xb8\x89\xea\xb8\x88'.encode().decode(), ascending=False)
