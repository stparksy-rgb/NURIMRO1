...
# ==================== 거래 내역 ====================
    ...
    if len(df) == 0:
        ...
        def 거래처별_미수금_계산(거래처명):
            """거래처별 현재 미수금 계산 (기초미수금 + 2025년 이후 거래)"""
            # 기초미수금 (2024.12.31 기준)
            ...
            # 2025년 이후 거래만 필터링
            기준일 = pd.Timestamp('2025-01-01')
            거래처_df = st.session_state.ledger_df[st.session_state.ledger_df['거래처'] == 거래처명]
            거래처_df_2025 = 거래처_df[거래처_df['날짜'] >= 기준일]
            
            # 2025년 이후 판매 (외출)
            판매 = 거래처_df_2025[거래처_df_2025['참조'].str.contains('외출', na=False)]['공급가액'].sum()
            판매_부가세 = 거래처_df_2025[거래처_df_2025['참조'].str.contains('외출', na=False)]['부가세'].sum()
            
            # 2025년 이후 입금
            입금 = 거래처_df_2025[거래처_df_2025['참조'].str.contains('입금', na=False)]['공급가액'].sum()
            
            미수금 = 기초미수금 + 판매 + 판매_부가세 - 입금
            return 미수금
...
# ==================== 외상 관리 ====================
    ...
    if len(df) == 0:
        ...
        st.caption("💡 미수금 = 기초미수금(2024.12.31) + 2025년 이후 판매 - 2025년 이후 입금")
        
        # 기초미수금 dict (2024.12.31 기준 - 컴장부 GULREST)
        기초미수금_dict = {}
        if len(base_rec) > 0:
            for _, row in base_rec.iterrows():
                기초미수금_dict[row['거래처']] = row['기초미수금']
        
        # 2025년 이후 거래만 필터링 (기초미수금 이후 거래)
        기준일 = pd.Timestamp('2025-01-01')
        df_2025이후 = df[df['날짜'] >= 기준일]
        
        # 최근 1년 거래 기준 거래처 목록
        기준일_1년 = get_kst_now() - timedelta(days=365)
        최근1년_df = df[df['날짜'] >= 기준일_1년]
        최근_거래처 = set(최근1년_df['거래처'].dropna().unique())
        
        # 기초미수금이 있는 거래처도 포함
        기초_거래처 = set(기초미수금_dict.keys())
        전체_거래처 = 최근_거래처.union(기초_거래처)
        
        # 거래처별 미수금 계산
        미수금_목록 = []
        
        for 거래처 in 전체_거래처:
            # 기초 미수금 (2024.12.31 기준)
            기초미수금 = 기초미수금_dict.get(거래처, 0)
            
            # 2025년 이후 거래만 계산
            거래처_df_2025 = df_2025이후[df_2025이후['거래처'] == 거래처]
            
            # 2025년 이후 판매 (외출)
            판매_df = 거래처_df_2025[거래처_df_2025['참조'].str.contains('외출', na=False)]
            공급가액_2025 = 판매_df['공급가액'].sum() if len(판매_df) > 0 else 0
            부가세_2025 = 판매_df['부가세'].sum() if len(판매_df) > 0 else 0
            
            # 2025년 이후 입금
            입금_df = 거래처_df_2025[거래처_df_2025['참조'].str.contains('입금', na=False)]
            입금액_2025 = 입금_df['공급가액'].sum() if len(입금_df) > 0 else 0
            
            # 미수금 계산: 기초미수금 + 2025년 판매 + 부가세 - 2025년 입금
            미수금 = 기초미수금 + 공급가액_2025 + 부가세_2025 - 입금액_2025
            
            # 최근 1년 거래 통계 (표시용)
            거래처_df_1년 = 최근1년_df[최근1년_df['거래처'] == 거래처]
            판매_df_1년 = 거래처_df_1년[거래처_df_1년['참조'].str.contains('외출', na=False)]
            입금_df_1년 = 거래처_df_1년[거래처_df_1년['참조'].str.contains('입금', na=False)]
            
            공급가액 = 판매_df_1년['공급가액'].sum() if len(판매_df_1년) > 0 else 0
            부가세 = 판매_df_1년['부가세'].sum() if len(판매_df_1년) > 0 else 0
            입금액 = 입금_df_1년['공급가액'].sum() if len(입금_df_1년) > 0 else 0
            
            # 미수금이 양수인 경우만 (받을 돈)
            if 미수금 > 0:
                전체_거래처_df = df[df['거래처'] == 거래처]
                최근거래일 = 전체_거래처_df['날짜'].max() if len(전체_거래처_df) > 0 else pd.NaT
...
# ==================== 설정 ====================
    ...
    # 탭 생성
    tab1, tab2, tab3, tab4 = st.tabs(["🏢 사업자 정보", "🗄️ 데이터 관리", "💰 기초 미수금 설정", "📊 통계"])
