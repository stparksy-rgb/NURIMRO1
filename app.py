...
# ==================== 거래 내역 ====================
    ...
    if len(df) == 0:
        ...
        def 거래처별_미수금_계산(거래처명):
            ...
            # 판매 (외출)
            판매 = 거래처_df[거래처_df['참조'].str.contains('외출', na=False)]['공급가액'].sum()
            판매_부가세 = 거래처_df[거래처_df['참조'].str.contains('외출', na=False)]['부가세'].sum()
            
            # 입금
            입금 = 거래처_df[거래처_df['참조'].str.contains('입금', na=False)]['공급가액'].sum()
...
# ==================== 외상 관리 ====================
    ...
    if len(df) == 0:
        ...
        st.caption("💡 미수금 = 기초미수금 + 판매(외출) + 부가세 - 입금 (실시간 계산)")
        ...
        # 모든 거래처 (기초미수금 + 거래내역)
        거래_거래처 = set(df['거래처'].dropna().unique())
        기초_거래처 = set(기초미수금_dict.keys())
        전체_거래처 = 거래_거래처.union(기초_거래처)
        ...
        for 거래처 in 전체_거래처:
            ...
            공급가액 = 판매_df['공급가액'].sum() if len(판매_df) > 0 else 0
            ...
            # 실시간 미수금 계산: 기초미수금 + 판매 + 부가세 - 입금
            미수금 = 기초미수금 + 공급가액 + 부가세 - 입금액
        ...
        if 미수금_목록:
            ...
            # 컬럼 순서 정리
            display_df = display_df[['거래처', '기초미수금', '공급가액', '부가세', '합계', '입금액', '미수금', '최근거래일']]
            
            # 금액 포맷팅
            for col in ['기초미수금', '공급가액', '부가세', '합계', '입금액', '미수금']:
