...
def calculate_receivable(거래처, df=None):
    ...
    총_판매 = 판매_df['공급가액'].sum() if len(판매_df) > 0 else 0
    총_부가세 = 판매_df['부가세'].sum() if len(판매_df) > 0 else 0
    
    # 전체 입금
    입금_df = 거래처_df[거래처_df['참조'].str.contains('입금', na=False)]
    총_입금 = 입금_df['공급가액'].sum() if len(입금_df) > 0 else 0
...
def calculate_all_receivables(df=None):
    ...
    for 거래처 in 전체_거래처:
        ...
        # 미수금이 있는 거래처만 (양수 = 받을 돈, 음수 = 선수금)
        if 미수금 != 0:
            # 최근 거래일
            최근거래일 = 거래처_df['날짜'].max() if len(거래처_df) > 0 else pd.NaT
...
def save_company_info():
    company_df = pd.DataFrame([st.session_state.company_info])
        ...
        <div class="invoice-section">
            ...
            <table class="info-table">
                ...
                <tr>
                    ...
                    <td></td>
                    <td class="label">사업자번호</td>
                    <td>{company.get('사업자번호', '301-03-55081')}</td>
                </tr>
                <tr>
...
menu_list = ["🏠 대시보드", "➕ 거래 입력", "📄 거래 내역", "📊 통계 분석", "💰 외상 관리", "🧾 회계 관리", "📦 품목 관리", "📋 재고 관리", "👥 거래처 관리", "📅 방문 일정", "⚙️ 설정"]
...
# ==================== 거래 입력 ====================
    ...
    with right_col:
        ...
        # 비고 입력란
        비고 = st.text_input("📝 비고", placeholder="특이사항 입력 (선택)")
    ...
    if 저장_버튼 or 저장_출력_버튼:
        if not 거래처:
            ...
            if 저장_출력_버튼:
                ...
                html_content = create_invoice_html(거래처, 거래일자.strftime('%Y-%m-%d'), 거래_목록)
...
# ==================== 거래 내역 ====================
    ...
    if len(df) == 0:
        ...
        def 거래처별_미수금_계산(거래처명):
            """거래처별 현재 미수금 = 기초미수금 + 2025년 판매 + 부가세 - 2025년 입금"""
            # 기초미수금 (없으면 0)
            ...
            # 2025년 이후 거래만
            기준일_2025 = pd.Timestamp('2025-01-01')
            거래처_df = st.session_state.ledger_df[st.session_state.ledger_df['거래처'] == 거래처명]
            거래처_df_2025 = 거래처_df[거래처_df['날짜'] >= 기준일_2025]
            
            # 2025년 이후 판매 (외출)
            판매_df = 거래처_df_2025[거래처_df_2025['참조'].str.contains('외출', na=False)]
            판매 = 판매_df['공급가액'].sum() if len(판매_df) > 0 else 0
            부가세 = 판매_df['부가세'].sum() if len(판매_df) > 0 else 0
            
            # 2025년 이후 입금
            입금_df = 거래처_df_2025[거래처_df_2025['참조'].str.contains('입금', na=False)]
            입금 = 입금_df['공급가액'].sum() if len(입금_df) > 0 else 0
...
# ==================== 외상 관리 ====================
    ...
    if len(df) == 0:
        ...
        st.caption("💡 미수금 = 전체 판매(외출) + 부가세 - 전체 입금 (컴장부 데이터 기반 자동 계산)")
        
        # 전체 거래처별 미수금 계산 (새로운 함수 사용)
        미수금_결과 = calculate_all_receivables(df)
        
        if len(미수금_결과) > 0:
            # 미수금만 필터링 (양수 = 받을 돈)
            미수금_df = 미수금_결과[미수금_결과['미수금'] > 0].copy()
            미수금_df = 미수금_df.sort_values('미수금', ascending=False)
            
            # 선수금 (음수 = 미리 받은 돈)
            선수금_df = 미수금_결과[미수금_결과['미수금'] < 0].copy()
            선수금_df = 선수금_df.sort_values('미수금', ascending=True)
            ...
            if len(미수금_df) > 0:
                ...
                display_df['최근거래일'] = pd.to_datetime(display_df['최근거래일']).dt.strftime('%Y-%m-%d')
                
                # 컬럼 순서 정리 및 이름 변경
                display_df = display_df[['거래처', '총_판매', '총_부가세', '합계', '총_입금', '미수금', '최근거래일']]
                display_df.columns = ['거래처', '총판매', '부가세', '합계', '입금액', '미수금', '최근거래일']
                
                # 금액 포맷팅
                for col in ['총판매', '부가세', '합계', '입금액', '미수금']:
            ...
            if len(선수금_df) > 0:
                st.markdown("---")
                st.markdown("### 💰 선수금 (미리 받은 돈)")
                
                선수금_df['선수금'] = 선수금_df['미수금'].abs()
                
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("총 선수금", f"{선수금_df['선수금'].sum():,.0f}원")
                with col2:
                    st.metric("선수금 거래처 수", f"{len(선수금_df)}개")
                
                display_선수금 = 선수금_df[['거래처', '선수금', '최근거래일']].copy()
                display_선수금['최근거래일'] = pd.to_datetime(display_선수금['최근거래일']).dt.strftime('%Y-%m-%d')
                display_선수금['선수금'] = display_선수금['선수금'].apply(lambda x: f"{x:,.0f}")
                
                st.dataframe(display_선수금, use_container_width=True, hide_index=True)
        else:
            st.success("✅ 미수금/선수금이 없습니다!")
                ...
                with tab1:
                    ...
                    if len(반복_거래처) == 0:
                        ...
                        for 거래처 in 반복_거래처:
                            ...
                            if 결과:
                                # 지역 정보 추가
                                지역_info = customers_df[customers_df['거래처명'] == 거래처]
                                if len(지역_info) > 0 and pd.notna(지역_info['지역'].values[0]) and 지역_info['지역'].values[0] != '':
                                    결과['지역'] = 지역_info['지역'].values[0]
                                else:
                                    결과['지역'] = '미지정'
                                방문_일정.append(결과)
...
# ==================== 설정 ====================
    ...
    # 탭 생성
    tab1, tab2, tab3, tab4 = st.tabs(["🏢 사업자 정보", "🗄️ 데이터 관리", "💰 미수금 현황", "📊 통계"])
        ...
        **✅ 자동 계산 방식:**
        ...
        if len(df) > 0:
            ...
            if len(미수금_결과) > 0:
                ...
                with col3:
                    총_선수금 = 선수금_df['미수금'].abs().sum() if len(선수금_df) > 0 else 0
                    st.metric("총 선수금", f"{총_선수금:,.0f}원")
                
                st.markdown("---")
                
                # 거래처별 미수금 검색
                st.markdown("#### 🔍 거래처별 미수금 조회")
                거래처_list = sorted(df['거래처'].dropna().unique().tolist())
                선택_거래처 = st.selectbox("거래처 선택", ["전체"] + 거래처_list, key="rec_search")
                
                if 선택_거래처 == "전체":
                    if len(미수금_df) > 0:
                        display_df = 미수금_df.sort_values('미수금', ascending=False).copy()
                        display_df['최근거래일'] = pd.to_datetime(display_df['최근거래일']).dt.strftime('%Y-%m-%d')
                        display_df = display_df[['거래처', '총_판매', '총_부가세', '총_입금', '미수금', '최근거래일']]
                        display_df.columns = ['거래처', '총판매', '부가세', '입금', '미수금', '최근거래일']
                        
                        for col in ['총판매', '부가세', '입금', '미수금']:
                            display_df[col] = display_df[col].apply(lambda x: f"{x:,.0f}")
                        
                        st.dataframe(display_df, use_container_width=True, hide_index=True)
                    else:
                        st.info("미수금이 있는 거래처가 없습니다.")
                else:
                    # 특정 거래처 상세
                    거래처_미수금 = calculate_receivable(선택_거래처)
                    
                    if 거래처_미수금 > 0:
                        st.warning(f"⚠️ **{선택_거래처}** 미수금: **{거래처_미수금:,.0f}원**")
                    elif 거래처_미수금 < 0:
                        st.info(f"💰 **{선택_거래처}** 선수금: **{abs(거래처_미수금):,.0f}원**")
                    else:
                        st.success(f"✅ **{선택_거래처}** 미수금 없음")
                    ...
                    if len(거래처_df) > 0:
                        ...
                        display_거래 = display_거래[['날짜', '품목', '참조', '공급가액', '부가세']]
                ...
                st.info("거래 데이터가 있지만 미수금/선수금이 없습니다.")
        else:
    ...
    with tab4:
        ...
        with col1:
            ...
            총_미수금 = 미수금_df['미수금'].sum() if len(미수금_df) > 0 else 0
            st.metric("총 미수금 (받을 돈)", f"{총_미수금:,.0f}원")
        with col2:
            선수금_df = 미수금_결과[미수금_결과['미수금'] < 0] if len(미수금_결과) > 0 else pd.DataFrame()
            총_선수금 = 선수금_df['미수금'].abs().sum() if len(선수금_df) > 0 else 0
            st.metric("총 선수금 (미리 받은 돈)", f"{총_선수금:,.0f}원")
