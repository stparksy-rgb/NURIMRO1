...
# ==================== 거래 입력 ====================
    ...
    if 거래처:
        ...
        with col_info1:
            st.markdown(f"""
            <div style='background-color: #1565c0; border: 3px solid #0d47a1; border-radius: 10px; padding: 15px;'>
                <h3 style='color: #ffffff; margin: 0; font-size: 20px;'>🏢 {거래처}</h3>
            </div>
            """, unsafe_allow_html=True)
        with col_info2:
            ...
            if 미수금 > 0:
                st.markdown(f"""
                <div style='background-color: #e65100; border: 3px solid #bf360c; border-radius: 10px; padding: 15px;'>
                    <h3 style='color: #ffffff; margin: 0; font-size: 20px;'>⚠️ 미수금: {미수금:,.0f}원</h3>
                </div>
                """, unsafe_allow_html=True)
            elif 미수금 < 0:
                st.markdown(f"""
                <div style='background-color: #2e7d32; border: 3px solid #1b5e20; border-radius: 10px; padding: 15px;'>
                    <h3 style='color: #ffffff; margin: 0; font-size: 20px;'>💰 선수금: {abs(미수금):,.0f}원</h3>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div style='background-color: #388e3c; border: 3px solid #1b5e20; border-radius: 10px; padding: 15px;'>
                    <h3 style='color: #ffffff; margin: 0; font-size: 20px;'>✅ 미수금 없음</h3>
                </div>
                """, unsafe_allow_html=True)
            ...
            if len(products_df) > 0 and 검색어:
                ...
                if len(검색결과) > 0:
                    ...
                    if 선택idx > 0:
                        ...
                        # ✅ 선택된 품목 크게 표시
                        st.markdown(f"""
                        <div style='background-color: #f57c00; border: 3px solid #e65100; border-radius: 10px; padding: 12px; margin: 10px 0;'>
                            <h3 style='color: #ffffff; margin: 0; font-size: 18px;'>📦 {품목명}</h3>
                        </div>
                        """, unsafe_allow_html=True)
                        
                        # ✅ 최근 판매가/매입가 조회 - 개선된 로직
                        품목_검색명 = 선택품목정보['품목명']
                        
                        # 최근 판매 내역 (외출) - 단가 조건 완화
                        판매내역 = df[
