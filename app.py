...
def calculate_receivable(ê±°ëž˜ì²˜, df=None):
    """ê±°ëž˜ì²˜ë³„ ë¯¸ìˆ˜ê¸ˆ ì‹¤ì‹œê°„ ê³„ì‚° (ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ + ê¸°ì¤€ì ¼ìž  ì ´í›„ ê±°ëž˜)"""
    if df is None:
...
def calculate_all_receivables(df=None):
    ...
    ê±°ëž˜ì²˜set = set()
    if len(base_recv) > 0:
        ê±°ëž˜ì²˜set.update(base_recv['ê±°ëž˜ì²˜'].unique())
    if len(df) > 0:
        ê±°ëž˜ì²˜set.update(df['ê±°ëž˜ì²˜'].dropna().unique())
    
    ê²°ê³¼ = []
    for ê±°ëž˜ì²˜ in ê±°ëž˜ì²˜set:
        ...
        if ë¯¸ìˆ˜ê¸ˆ > 0:
            ...
            if len(ê±°ëž˜ì²˜df) > 0:
                ...
                ê±°ëž˜ì²˜_ê¸°ì´ˆ = base_recv[base_recv['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜] if len(base_recv) > 0 else pd.DataFrame()
