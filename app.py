    ...
    if len(df) == 0:
        ...
        def 누적_미수금_계산(df):
            ...
            for idx, row in df_sorted.iterrows():
                ...
                # 해당 거래처 첫 계산시 기초미수금으로 초기화
                if 거래처 not in 현재_미수금:
                    현재_미수금[거래처] = 기초미수금_dict[거래처]
                
                # 기준일자 이전 거래는 기초미수금만 표시
        ...
        if '합계' not in display_df.columns:
            display_df['합계'] = display_df['공급가액'] + display_df['부가세']
        ...
        display_df['합계'] = display_df['합계'].apply(lambda x: f"{x:,.0f}")
