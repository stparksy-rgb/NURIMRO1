        ...
        with subtab1:
            if len(prospects_df) > 0:
                ...
                with col_f1:
                    ...
                    선택_지역 = st.selectbox("지역", 지역_목록, key="filter_region")
                ...
                for idx, row in 페이지_df.iterrows():
                    ...
                    단계_아이콘 = 단계_색상.get(row.get('영업단계', '미방문'), '🔵')
                    
                    with st.expander(f"{단계_아이콘} **{row['업체명']}** | {row['지역']} | {row['업종']}"):
                        col_info, col_action = st.columns([2, 1])
                        
                        with col_info:
                            st.markdown(f"**📍 주소:** {row.get('주소', '')}")
        ...
        if len(prospects_df) > 0:
            ...
            with col_dl2:
                ...
                if st.button("🗑️ 전체 데이터 삭제", type="secondary"):
                    st.warning("⚠️ 정말 모든 잠재거래처를 삭제하시겠습니까?")
                    if st.button("⚠️ 예, 전체 삭제합니다", type="secondary", key="confirm_delete_all"):
