...
# ==================== 거래 내역 ====================
    ...
    if len(df) == 0:
        ...
        # 미수금 실시간 표시 - calculate_receivable로 계산 (공급가액+부가세-입금)
        if "전체" not in 거래처_필터 and len(거래처_필터) == 1:
            선택거래처 = 거래처_필터[0]
            
            # 미수금은 calculate_receivable로 계산 (공급가액+부가세-입금)
            미수금 = calculate_receivable(선택거래처)
        ...
        # 미수금 계산 함수 - calculate_receivable 사용
        def 거래처별_미수금_계산(거래처명):
            """거래처별 현재 미수금 = 전체 판매 + 부가세 - 전체 입금"""
            return calculate_receivable(거래처명)
    ...
    if len(df) == 0:
        ...
        with tab5:
            ...
            if len(유영찬_df) == 0:
                ...
                # 미수금 표시 - calculate_receivable 사용
                유영찬_미수금 = calculate_receivable('유영찬')
...
# ==================== 거래처 관리 ====================
    ...
    with tab1:
        ...
        if len(고객_매출) > 0:
            ...
            for i, item in enumerate(고객_매출[:5]):
                ...
                # 미수금은 calculate_receivable로 계산 (공급가액+부가세-입금)
                미수금 = calculate_receivable(거래처명, ledger_df)
    ...
    with tab2:
        ...
        if len(전체_고객) > 0:
            ...
            # ===== 미수금 현황 (calculate_receivable로 계산) =====
            st.markdown("### 💰 미수금 현황")
            
            # 미수금은 calculate_receivable로 계산 (공급가액+부가세-입금)
            미수금 = calculate_receivable(선택_고객, ledger_df)
