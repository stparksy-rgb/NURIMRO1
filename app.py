...
# ==================== 외상 관리 ====================
    ...
    if len(base_recv) == 0:
        ...
        with tab_recv:
            ...
            st.caption("💡 미수금 = 컴장부 GULREST 값 (실시간 잔액)")
            ...
            # 선수금 (음수를 양수로 표시)
            선수금_raw = base_recv[base_recv['기초미수금'] < 0].copy()
            ...
            if len(미수금_df) > 0:
                ...
                # 최근 1년 거래 정보 추가
                최근1년 = get_kst_now() - timedelta(days=365)
                
                미수금_상세 = []
                for _, row in 미수금_df.iterrows():
                    거래처 = row['거래처']
                    미수금 = row['기초미수금']
                    
                    # 최근 1년 거래 정보
                    거래처_df = df[(df['거래처'] == 거래처) & (df['날짜'] >= 최근1년)]
                    외출_df = 거래처_df[거래처_df['참조'].str.contains('외출', na=False)]
                    입금_df = 거래처_df[거래처_df['참조'].str.contains('입금', na=False)]
                    
                    공급가액 = 외출_df['공급가액'].sum() if len(외출_df) > 0 else 0
                    부가세 = 외출_df['부가세'].sum() if len(외출_df) > 0 else 0
                    입금액 = 입금_df['공급가액'].sum() if len(입금_df) > 0 else 0
                ...
                display_df = pd.DataFrame(미수금_상세)
            ...
            if len(선수금_raw) > 0:
                ...
                선수금_df = 선수금_raw.copy()
                선수금_df['선수금'] = 선수금_df['기초미수금'].abs()
                선수금_df = 선수금_df.sort_values('선수금', ascending=False)
                ...
                display_선수금 = 선수금_df[['거래처', '선수금']].copy()
        ...
        with tab_pay:
            ...
            st.caption("💡 미지급금 = 컴장부 GULREST 값 (음수 = 줄 돈)")
            
            # base_receivables에서 미지급금 가져오기 (음수 = 줄 돈)
            ...
            if len(미지급금_raw) > 0:
                ...
                미지급금_상세 = []
                for _, row in 미지급금_df.iterrows():
                    거래처 = row['거래처']
                    미지급금 = row['미지급금']
                    
                    # 최근 1년 거래 정보
                    거래처_df = df[(df['거래처'] == 거래처) & (df['날짜'] >= 최근1년)]
                    외입_df = 거래처_df[거래처_df['참조'].str.contains('외입', na=False)]
                    출금_df = 거래처_df[거래처_df['참조'].str.contains('출금', na=False)]
                    
                    매입가액 = abs(외입_df['공급가액'].sum()) if len(외입_df) > 0 else 0
                    부가세 = abs(외입_df['부가세'].sum()) if len(외입_df) > 0 else 0
                    출금액 = abs(출금_df['공급가액'].sum()) if len(출금_df) > 0 else 0
                ...
                display_df = pd.DataFrame(미지급금_상세)
                display_df['최근거래일'] = pd.to_datetime(display_df['최근거래일']).dt.strftime('%Y-%m-%d')
                
                for col in ['매입가액', '부가세', '합계', '출금액', '미지급금']:
                    display_df[col] = display_df[col].apply(lambda x: f"{x:,.0f}")
