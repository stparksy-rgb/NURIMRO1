    ...
    if len(df) == 0:
        ...
        if len(판매_df) == 0:
            ...
            with tab1:
                ...
                with col1:
                    선택_연도 = st.selectbox("연도", [2025, 2026], index=0, key="visit_year")
                ...
                for 거래처 in 반복_거래처:
                    ...
                    if 결과:
                        ...
                        결과['지역'] = 지역_info['지역'].values[0] if len(지역_info) > 0 and pd.notna(지역_info['지역'].values[0]) else '미지정'
                        방문_일정.append(결과)
                ...
                if 방문_일정:
                    ...
                    # 해당 월 방문 예상 또는 이미 지난 거래처
                    월간_일정 = 일정_df[
                        (일정_df['다음예상'] <= 월_끝) | 
                        (일정_df['다음예상'] < get_kst_now())
                    ].copy()
                            ...
                            with st.expander(f"📍 {지역} ({len(지역_df)}개 업체)", expanded=True):
                                for 주차 in sorted(지역_df['주차'].unique()):
                                    ...
                                    for _, row in 주차_df.iterrows():
                                        ...
                                        상태 = "🔴 지남" if row['다음예상'] < get_kst_now() else "🟢 예정"
                    ...
                    st.info("2회 이상 반복 거래한 거래처가 없습니다.")
            ...
            with tab2:
                ...
                if 방문_일정:
                    ...
                    일정_df['상태'] = 일정_df['다음예상'].apply(
                        lambda x: '🔴 방문필요' if x < get_kst_now() else '🟢 예정'
                    )
                    ...
                    with col2:
                        방문필요 = len(일정_df[일정_df['다음예상'] < get_kst_now()])
