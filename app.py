...
# ==================== 거래 입력 ====================
    ...
    if 거래처:
        ...
        with col_info1:
            st.markdown(f"""
            <div style='background-color: #e3f2fd; border: 2px solid #1e88e5; border-radius: 8px; padding: 10px;'>
                <h4 style='color: #1565c0; margin: 0;'>🏢 거래처: {거래처}</h4>
            </div>
            """, unsafe_allow_html=True)
        with col_info2:
            ...
            if 미수금 > 0:
                st.markdown(f"""
                <div style='background-color: #fff3e0; border: 2px solid #ff9800; border-radius: 8px; padding: 10px;'>
                    <h4 style='color: #e65100; margin: 0;'>⚠️ 미수금: {미수금:,.0f}원</h4>
                </div>
                """, unsafe_allow_html=True)
            elif 미수금 < 0:
                st.markdown(f"""
                <div style='background-color: #e8f5e9; border: 2px solid #4caf50; border-radius: 8px; padding: 10px;'>
                    <h4 style='color: #2e7d32; margin: 0;'>💰 선수금: {abs(미수금):,.0f}원</h4>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.success("✅ 미수금 없음")
            ...
            if len(products_df) > 0 and 검색어:
                ...
                if len(검색결과) > 0:
                    ...
                    if 선택idx > 0:
                        ...
                        # 최근 판매 내역 (외출)
                        판매내역 = df[
                            (df['품목'].str.contains(품목_검색명, na=False)) &
                            (df['참조'] == '=외출') &
                            (df['단가'] > 0)
                        ...
                        with col_price1:
                            if len(판매내역) > 0:
                                st.markdown("**🔵 최근 판매가 (외출)**")
                                for _, row in 판매내역.head(3).iterrows():
                                    날짜_str = row['날짜'].strftime('%m/%d') if pd.notna(row['날짜']) else ''
                                    거래처_str = str(row.get('거래처', ''))[:8]
                                    st.markdown(f"""
                                    <div style='background-color: #e3f2fd; border-radius: 5px; padding: 5px; margin: 3px 0; font-size: 12px;'>
                                        📅 {날짜_str} | {거래처_str} | <b style='color:#1565c0;'>단가: {row['단가']:,.0f}원</b>
                                    </div>
                                    """, unsafe_allow_html=True)
                                # 최근 판매 단가를 기본값으로 설정
                                st.session_state.선택품목_최근단가 = int(판매내역.iloc[0]['단가'])
                            else:
                                st.info("판매 이력 없음")
                                st.session_state.선택품목_최근단가 = 0
                        ...
                        with col_price2:
                            if len(매입내역) > 0:
                                st.markdown("**🟠 최근 매입가 (외입)**")
                                for _, row in 매입내역.head(3).iterrows():
                                    날짜_str = row['날짜'].strftime('%m/%d') if pd.notna(row['날짜']) else ''
                                    거래처_str = str(row.get('거래처', ''))[:8]
                                    st.markdown(f"""
                                    <div style='background-color: #fff3e0; border-radius: 5px; padding: 5px; margin: 3px 0; font-size: 12px;'>
                                        📅 {날짜_str} | {거래처_str} | <b style='color:#e65100;'>단가: {abs(row['단가']):,.0f}원</b>
                                    </div>
                                    """, unsafe_allow_html=True)
                            else:
                                st.info("매입 이력 없음")
