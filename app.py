...
def calculate_receivable(ê±°ëž˜ì²˜, df=None):
    """ê±°ëž˜ì²˜ë³„ ë¯¸ìˆ˜ê¸ˆ ê³„ì‚° - ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆì—  ë“±ë¡ ë œ ê±°ëž˜ì²˜ë§Œ!"""
    if df is None:
        df = st.session_state.ledger_df
    
    base_recv = st.session_state.base_receivables_df
    
    # ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆì—  ë“±ë¡ ë ˜ì§€ ì•Šì € ê±°ëž˜ì²˜ëŠ” 0 ë°˜í™˜
    if len(base_recv) == 0:
        return 0
    
    ê±°ëž˜ì²˜_ê¸°ì´ˆ = base_recv[base_recv['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜]
    if len(ê±°ëž˜ì²˜_ê¸°ì´ˆ) == 0:
        return 0  # ë“±ë¡  ì•ˆë œ ê±°ëž˜ì²˜ëŠ” ë¯¸ìˆ˜ê¸ˆ 0
    
    ê¸°ì´ˆê°’ = ê±°ëž˜ì²˜_ê¸°ì´ˆ['ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ'].values[0]
    if ê¸°ì´ˆê°’ <= 0:  # ì Œìˆ˜ë©´ ë¯¸ì§€ê¸‰ê¸ˆ, 0ì ´ë©´ ë¯¸ìˆ˜ê¸ˆ ì—†ì Œ
        return 0
    
    ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ = ê¸°ì´ˆê°’
    
    # ê¸°ì¤€ì ¼ìž  íŒŒì‹±
    try:
        ê¸°ì¤€ì ¼ìž  = pd.to_datetime(str(ê±°ëž˜ì²˜_ê¸°ì´ˆ['ê¸°ì¤€ì ¼ìž '].values[0]))
    except:
        return ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ  # ê¸°ì¤€ì ¼ìž  íŒŒì‹± ì‹¤íŒ¨ì‹œ ê¸°ì´ˆê°’ë§Œ ë°˜í™˜
    
    if len(df) == 0:
        return ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ
    
    ê±°ëž˜ì²˜df = df[df['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜].copy()
    if len(ê±°ëž˜ì²˜df) == 0:
        return ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ
    
    # ê¸°ì¤€ì ¼ìž  ì ´í›„ ê±°ëž˜ë§Œ í•„í„°ë§ 
    ê±°ëž˜ì²˜df['ë‚ ì§œ'] = pd.to_datetime(ê±°ëž˜ì²˜df['ë‚ ì§œ'])
    ê±°ëž˜ì²˜df = ê±°ëž˜ì²˜df[ê±°ëž˜ì²˜df['ë‚ ì§œ'] > ê¸°ì¤€ì ¼ìž ]
    
    if len(ê±°ëž˜ì²˜df) == 0:
        return ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ
    
    # íŒ ë§¤/ìž…ê¸ˆ ê³„ì‚°
    íŒ ë§¤m = (ê±°ëž˜ì²˜df['ì°¸ì¡°'] == '=ì™¸ì¶œ') | ((ê±°ëž˜ì²˜df['ê³µê¸‰ê°€ì•¡'] > 0) & (~ê±°ëž˜ì²˜df['ì°¸ì¡°'].fillna('').str.contains('ìž…ê¸ˆ|ì¶œê¸ˆ')))
    ìž…ê¸ˆm = (ê±°ëž˜ì²˜df['ì°¸ì¡°'] == '=ìž…ê¸ˆ') | (ê±°ëž˜ì²˜df['ì°¸ì¡°'].fillna('').str.contains('ìž…ê¸ˆ'))
    
    ì´ íŒ ë§¤ = ê±°ëž˜ì²˜df.loc[íŒ ë§¤m, 'ê³µê¸‰ê°€ì•¡'].sum()
    ì´ ë¶€ê°€ì„¸ = ê±°ëž˜ì²˜df.loc[íŒ ë§¤m, 'ë¶€ê°€ì„¸'].sum()
    ì´ ìž…ê¸ˆ = ê±°ëž˜ì²˜df.loc[ìž…ê¸ˆm, 'ê³µê¸‰ê°€ì•¡'].sum()
    
    return ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ + ì´ íŒ ë§¤ + ì´ ë¶€ê°€ì„¸ - ì´ ìž…ê¸ˆ

def calculate_payable(ê±°ëž˜ì²˜, df=None):
    """ê±°ëž˜ì²˜ë³„ ë¯¸ì§€ê¸‰ê¸ˆ ê³„ì‚° - ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆì—  ë“±ë¡ ë œ ê±°ëž˜ì²˜ë§Œ!"""
    if df is None:
        df = st.session_state.ledger_df
    ...
    ê±°ëž˜ì²˜_ê¸°ì´ˆ = base_recv[base_recv['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜]
    if len(ê±°ëž˜ì²˜_ê¸°ì´ˆ) == 0:
        return 0  # ë“±ë¡  ì•ˆë œ ê±°ëž˜ì²˜ëŠ” ë¯¸ì§€ê¸‰ê¸ˆ 0
    
    ê¸°ì´ˆê°’ = ê±°ëž˜ì²˜_ê¸°ì´ˆ['ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ'].values[0]
    if ê¸°ì´ˆê°’ >= 0:  # ì–‘ìˆ˜ë©´ ë¯¸ìˆ˜ê¸ˆ, 0ì ´ë©´ ë¯¸ì§€ê¸‰ê¸ˆ ì—†ì Œ
        return 0
    
    ê¸°ì´ˆë¯¸ì§€ê¸‰ê¸ˆ = abs(ê¸°ì´ˆê°’)
    
    try:
        ê¸°ì¤€ì ¼ìž  = pd.to_datetime(str(ê±°ëž˜ì²˜_ê¸°ì´ˆ['ê¸°ì¤€ì ¼ìž '].values[0]))
    except:
        return ê¸°ì´ˆë¯¸ì§€ê¸‰ê¸ˆ
    ...
    ë§¤ìž…m = (ê±°ëž˜ì²˜df['ì°¸ì¡°'] == '=ì™¸ìž…') | ((ê±°ëž˜ì²˜df['ê³µê¸‰ê°€ì•¡'] < 0) & (~ê±°ëž˜ì²˜df['ì°¸ì¡°'].fillna('').str.contains('ì¶œê¸ˆ|ìž…ê¸ˆ')))
    ì¶œê¸ˆm = (ê±°ëž˜ì²˜df['ì°¸ì¡°'] == '=ì¶œê¸ˆ') | (ê±°ëž˜ì²˜df['ì°¸ì¡°'].fillna('').str.contains('ì¶œê¸ˆ'))
    
    ì´ ë§¤ìž… = abs(ê±°ëž˜ì²˜df.loc[ë§¤ìž…m, 'ê³µê¸‰ê°€ì•¡'].sum())
    ì´ ë¶€ê°€ì„¸ = abs(ê±°ëž˜ì²˜df.loc[ë§¤ìž…m, 'ë¶€ê°€ì„¸'].sum())
    ì´ ì¶œê¸ˆ = abs(ê±°ëž˜ì²˜df.loc[ì¶œê¸ˆm, 'ê³µê¸‰ê°€ì•¡'].sum())
    
    return ê¸°ì´ˆë¯¸ì§€ê¸‰ê¸ˆ + ì´ ë§¤ìž… + ì´ ë¶€ê°€ì„¸ - ì´ ì¶œê¸ˆ

def calculate_all_receivables(df=None):
    """ì „ì²´ ë¯¸ìˆ˜ê¸ˆ - ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆì—  ë“±ë¡ ë œ ê±°ëž˜ì²˜ë§Œ!"""
    if df is None:
        df = st.session_state.ledger_df
    base_recv = st.session_state.base_receivables_df.copy()
    
    # ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆì—  ë“±ë¡ ë œ ê±°ëž˜ì²˜ë§Œ (ì–‘ìˆ˜ì ¸ ê²½ìš°ë§Œ)
    if len(base_recv) == 0:
        return pd.DataFrame(columns=['ê±°ëž˜ì²˜', 'ë¯¸ìˆ˜ê¸ˆ', 'ìµœê·¼ê±°ëž˜ì ¼'])
    
    ë¯¸ìˆ˜ê¸ˆ_ê±°ëž˜ì²˜ = base_recv[base_recv['ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ'] > 0]['ê±°ëž˜ì²˜'].unique()
    
    ê²°ê³¼ = []
    for ê±°ëž˜ì²˜ in ë¯¸ìˆ˜ê¸ˆ_ê±°ëž˜ì²˜:
        ë¯¸ìˆ˜ê¸ˆ = calculate_receivable(ê±°ëž˜ì²˜, df)
        if ë¯¸ìˆ˜ê¸ˆ > 0:
            ê±°ëž˜ì²˜df = df[df['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜] if len(df) > 0 else pd.DataFrame()
            if len(ê±°ëž˜ì²˜df) > 0:
                ìµœê·¼ì ¼ = pd.to_datetime(ê±°ëž˜ì²˜df['ë‚ ì§œ']).max().strftime('%Y-%m-%d')
            else:
                ê±°ëž˜ì²˜_ê¸°ì´ˆ = base_recv[base_recv['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜]
                ìµœê·¼ì ¼ = ê±°ëž˜ì²˜_ê¸°ì´ˆ['ê¸°ì¤€ì ¼ìž '].values[0] if len(ê±°ëž˜ì²˜_ê¸°ì´ˆ) > 0 else ''
            ê²°ê³¼.append({'ê±°ëž˜ì²˜': ê±°ëž˜ì²˜, 'ë¯¸ìˆ˜ê¸ˆ': ë¯¸ìˆ˜ê¸ˆ, 'ìµœê·¼ê±°ëž˜ì ¼': ìµœê·¼ì ¼})
    
    if not ê²°ê³¼:
        return pd.DataFrame(columns=['ê±°ëž˜ì²˜', 'ë¯¸ìˆ˜ê¸ˆ', 'ìµœê·¼ê±°ëž˜ì ¼'])
    return pd.DataFrame(ê²°ê³¼).sort_values('ë¯¸ìˆ˜ê¸ˆ', ascending=False)

def calculate_all_payables(df=None):
    """ì „ì²´ ë¯¸ì§€ê¸‰ê¸ˆ - ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆì—  ë“±ë¡ ë œ ê±°ëž˜ì²˜ë§Œ!"""
    if df is None:
        df = st.session_state.ledger_df
    base_recv = st.session_state.base_receivables_df.copy()
    
    if len(base_recv) == 0:
        return pd.DataFrame(columns=['ê±°ëž˜ì²˜', 'ë¯¸ì§€ê¸‰ê¸ˆ', 'ìµœê·¼ê±°ëž˜ì ¼'])
    
    ë¯¸ì§€ê¸‰ê¸ˆ_ê±°ëž˜ì²˜ = base_recv[base_recv['ê¸°ì´ˆë¯¸ìˆ˜ê¸ˆ'] < 0]['ê±°ëž˜ì²˜'].unique()
    
    ê²°ê³¼ = []
    for ê±°ëž˜ì²˜ in ë¯¸ì§€ê¸‰ê¸ˆ_ê±°ëž˜ì²˜:
        ë¯¸ì§€ê¸‰ê¸ˆ = calculate_payable(ê±°ëž˜ì²˜, df)
        if ë¯¸ì§€ê¸‰ê¸ˆ > 0:
            ê±°ëž˜ì²˜df = df[df['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜] if len(df) > 0 else pd.DataFrame()
            if len(ê±°ëž˜ì²˜df) > 0:
                ìµœê·¼ì ¼ = pd.to_datetime(ê±°ëž˜ì²˜df['ë‚ ì§œ']).max().strftime('%Y-%m-%d')
                ...
                ê±°ëž˜ì²˜_ê¸°ì´ˆ = base_recv[base_recv['ê±°ëž˜ì²˜'] == ê±°ëž˜ì²˜]
                ìµœê·¼ì ¼ = ê±°ëž˜ì²˜_ê¸°ì´ˆ['ê¸°ì¤€ì ¼ìž '].values[0] if len(ê±°ëž˜ì²˜_ê¸°ì´ˆ) > 0 else ''
            ê²°ê³¼.append({'ê±°ëž˜ì²˜': ê±°ëž˜ì²˜, 'ë¯¸ì§€ê¸‰ê¸ˆ': ë¯¸ì§€ê¸‰ê¸ˆ, 'ìµœê·¼ê±°ëž˜ì ¼': ìµœê·¼ì ¼})
    
    if not ê²°ê³¼:
        return pd.DataFrame(columns=['ê±°ëž˜ì²˜', 'ë¯¸ì§€ê¸‰ê¸ˆ', 'ìµœê·¼ê±°ëž˜ì ¼'])
    return pd.DataFrame(ê²°ê³¼).sort_values('ë¯¸ì§€ê¸‰ê¸ˆ', ascending=False)
