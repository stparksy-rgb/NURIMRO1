...
menu_list = ["🏠 대시보드", "➕ 거래 입력", "📄 거래 내역", "📊 통계 분석", "💰 외상 관리", "📦 품목 관리", "📋 재고 관리", "👥 거래처 관리", "⚙️ 설정"]
...
if menu == "🏠 대시보드":
    ...
    if len(df) == 0:
        ...
        # 최신월 기본값 설정
        최신월_시작 = df['날짜'].max().replace(day=1).date()
        최신월_끝 = df['날짜'].max().date()
        
        # 날짜 필터
        col1, col2 = st.columns(2)
        with col1:
            start_date = st.date_input("시작일", value=최신월_시작)
        with col2:
            end_date = st.date_input("종료일", value=최신월_끝)
...
# ==================== 재고 관리 ====================
    ...
    with tab1:
        ...
        if len(inventory_df) > 0:
            ...
            with col3:
                안전재고_미달 = len(inventory_df[inventory_df['현재재고'] < inventory_df['안전재고']]) if '안전재고' in inventory_df.columns else 0
                st.metric("⚠️ 재고 부족", f"{안전재고_미달}개")
            with col4:
            ...
            # 재고 목록 표시
            st.dataframe(
                display_df[['품목명', '현재재고', '기준일자']].rename(columns={
                    '현재재고': '재고수량'
                }),
    ...
    with tab3:
        ...
        if len(inventory_df) > 0:
            ...
            부족_df = inventory_df[inventory_df['현재재고'] <= 부족_기준].sort_values('현재재고')
            
            if len(부족_df) > 0:
                st.error(f"⚠️ 재고 부족 품목: **{len(부족_df)}개**")
                ...
                for _, row in 부족_df.iterrows():
                    ...
                    if 재고 <= 0:
                        st.markdown(f"❌ **{품목}** - 재고 없음!")
                    elif 재고 <= 부족_기준 / 2:
                        st.markdown(f"🔴 **{품목}** - {재고:,.0f}개 (긴급)")
                    else:
                        st.markdown(f"🟡 **{품목}** - {재고:,.0f}개")
            else:
...
# ==================== 거래처 관리 ====================
    ...
    with tab1:
        ...
        # 고객업체만 대상으로 (내 고객!) - 제외 거래처 빼기
        고객_list = [x[0] for x in 분류된_거래처['고객_활성'] if x[0] not in 제외_거래처]
            ...
            if 거래처_기대치:
                for i, item in enumerate(거래처_기대치[:5]):
                    ...
                    with st.container():
                        ...
                        with col2:
                            st.markdown(f"💰 미수금: **{미수금:,.0f}원** | 월평균: **{상세['월평균금액']:,.0f}원**")
                        with col3:
                            st.markdown(f"**{배지}** ({점수:.0f}점)")
