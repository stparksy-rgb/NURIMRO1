...
# ==================== 거래처 관리 ====================
    ...
    with tab4:
        ...
        with sub_tab1:
            ...
            # 기존 거래처 선택 또는 신규 입력
            ledger_data = st.session_state.get('ledger_df', pd.DataFrame())
            기존_거래처_list = sorted(ledger_data['거래처'].unique().tolist()) if len(ledger_data) > 0 and '거래처' in ledger_data.columns else []
