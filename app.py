    ...
    with left_col:
        ...
        if 품목입력방식 == "품목 검색":
            if len(products_df) > 0:
                # 검색 필터
                검색어 = st.text_input("품목 검색", placeholder="품목명 또는 코드 입력 (예: 절단석, TUR)", label_visibility="collapsed")
                ...
                if len(검색결과) > 0:
                    ...
                    선택품목 = st.selectbox("품목 선택", ["선택하세요"] + 품목_옵션, key="search_result", label_visibility="collapsed")
...
# ==================== 품목 관리 ====================
    ...
    with tab1:
        ...
        if len(products_df) > 0:
            ...
            with col2:
                카테고리_개수 = len([x for x in products_df['카테고리'].unique() if pd.notna(x)])
                st.metric("카테고리 수", f"{카테고리_개수}개")
            with col3:
