...
# ==================== 거래 내역 ====================
    ...
    with st.expander("➕ 빠른 거래 입력", expanded=False):
        ...
        with col3:
            입력_거래유형 = st.selectbox("유형", ["=외입", "=입금", "=출금", "=외출"], key="quick_type")
        ...
        with col1:
            부가세_적용 = st.checkbox("부가세 10%", value=True if 입력_거래유형 in ["=외입", "=외출"] else False, key="quick_tax")
            ...
            if st.button("💾 저장", type="primary", use_container_width=True, key="quick_save"):
                if not 입력_거래처:
                    ...
                    new_row = pd.DataFrame([{
                        ...
                        '거래처': 입력_거래처,
                        '품목': 입력_품목 if 입력_품목 else 입력_거래유형.replace("=", ""),
                        '수량': 입력_수량,
                        '단가': 입력_단가,
                        '공급가액': 입력_공급가액,
                        '부가세': 입력_부가세,
