    ...
    if len(df) == 0:
        ...
        with tab2:
            ...
            for 월 in range(1, 13):
                ...
                총매출액 = 외출_df['공급가액'].sum() + 외출_df['부가세'].sum()  # 부가세 포함 총액
                계산서매출 = 외출_df[외출_df['부가세'] > 0]['공급가액'].sum()  # 부가세 있는 거래만
                매출부가세 = 외출_df['부가세'].sum()
                현금매출 = 외출_df[외출_df['부가세'] == 0]['공급가액'].sum()  # 부가세 없는 거래
            ...
            # 연간 합계
            연간_총매출 = 월별_df['총매출액'].sum()
            연간_계산서매출 = 월별_df['계산서매출'].sum()
            연간_매출부가세 = 월별_df['매출부가세'].sum()
            연간_현금매출 = 월별_df['현금매출'].sum()
            연간_매입액 = 월별_df['매입액'].sum()
            연간_매입부가세 = 월별_df['매입부가세'].sum()
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("총매출액", f"{연간_총매출:,.0f}원")
            with col2:
                st.metric("계산서매출", f"{연간_계산서매출:,.0f}원")
            with col3:
                st.metric("현금매출", f"{연간_현금매출:,.0f}원")
            with col4:
                if 연간_납부세액 >= 0:
            ...
            with col3:
                st.metric("차액 (납부세액)", f"{연간_납부세액:,.0f}원")
            ...
            for col in ['총매출액', '계산서매출', '매출부가세', '현금매출', '매입액', '매입부가세', '납부세액', '누적납부']:
                display_df[col] = display_df[col].apply(lambda x: f"{x:,.0f}")
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(name='매출부가세', x=월별_df['월'], y=월별_df['매출부가세'], marker_color='#1976D2'))
            fig.add_trace(go.Bar(name='매입부가세', x=월별_df['월'], y=월별_df['매입부가세'], marker_color='#FF5722'))
            fig.add_trace(go.Scatter(name='납부세액', x=월별_df['월'], y=월별_df['납부세액'], mode='lines+markers', line=dict(color='#43A047', width=3)))
            fig.update_layout(barmode='group', height=400)
            st.plotly_chart(fig, use_container_width=True)
