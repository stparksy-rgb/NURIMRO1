...
# 데이터 파일 경로
# GitHub에서 직접 읽기
GITHUB_BASE = "https://raw.githubusercontent.com/stparksy-rgb/NURIMRO1/main/data"
...
if True:  # 항상 최신 파일 읽기
    if os.path.exists(DATA_FILE):
        try:
            try:
                st.session_state.ledger_df = pd.read_csv(f"{GITHUB_BASE}/ledger.csv")
            except:
...
# ==================== 거래 내역 ====================
    ...
    if len(df) == 0:
        ...
        **데이터가 표시되지 않는 경우 확인하세요:**
        ...
        with col1:
            참조_옵션 = [x for x in df['참조'].dropna().unique() if str(x).strip() != '']
            거래유형_필터 = st.multiselect("거래 유형", 참조_옵션, default=참조_옵션)
        with col2:
        ...
        # 미수금 포맷팅
        if '미수금' not in display_df.columns:
            display_df['미수금'] = ''
        else:
