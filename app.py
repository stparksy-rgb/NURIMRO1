...
def calculate_receivable(거래처, df=None):
    ...
    - 판매: 공급가액 > 0, 내용에 '입금' 없음
    - 입금: 공급가액 > 0, 내용에 '입금' 포함
    - 매입: 공급가액 < 0, 내용에 '입금/출금' 없음
    - 출금: 공급가액 < 0, 내용에 '입금' 또는 '출금' 포함
...
def calculate_all_receivables(df=None):
    ...
    for 거래처 in 전체_거래처:
        ...
        # 판매 거래가 있는 거래처만 (미수금이 있거나 선수금이 있는 경우)
        if 총_판매 > 0 or 미수금 != 0:
...
# ==================== 외상 관리 ====================
    ...
    if len(df) == 0:
        ...
        with tab_recv:
            ...
            st.caption("💡 미수금 = 판매(양수) + 부가세 - 입금 | 판매처에서 받을 돈")
            ...
            if len(미수금_결과) > 0:
                ...
                st.info("판매 거래 내역이 없습니다.")
        
        # ===== 미지급금 탭 (매입처) =====
        with tab_pay:
            st.markdown("### 📥 외상 매입금 (미지급금)")
            st.caption("💡 미지급금 = |매입(음수)| + |부가세| - |출금| | 매입처에 줄 돈")
            
            미지급금_결과 = calculate_all_payables(df)
            
            if len(미지급금_결과) > 0:
                # 미지급금만 필터링 (양수 = 줄 돈)
                미지급금_df = 미지급금_결과[미지급금_결과['미지급금'] > 0].copy()
                미지급금_df = 미지급금_df.sort_values('미지급금', ascending=False)
                
                # 선급금 (음수 = 미리 준 돈)
                선급금_df = 미지급금_결과[미지급금_결과['미지급금'] < 0].copy()
                선급금_df = 선급금_df.sort_values('미지급금', ascending=True)
                ...
                if len(미지급금_df) > 0:
                    ...
                    # 상세 내역 표시
                    display_df = 미지급금_df.copy()
                    display_df['최근거래일'] = pd.to_datetime(display_df['최근거래일']).dt.strftime('%Y-%m-%d')
                    
                    display_df = display_df[['거래처', '총_매입', '총_부가세', '합계', '총_출금', '미지급금', '최근거래일']]
                    display_df.columns = ['거래처', '총매입', '부가세', '합계', '출금액', '미지급금', '최근거래일']
                    
                    for col in ['총매입', '부가세', '합계', '출금액', '미지급금']:
                        display_df[col] = display_df[col].apply(lambda x: f"{x:,.0f}")
                ...
                if len(선급금_df) > 0:
                    st.markdown("---")
                    st.markdown("### 💵 선급금 (미리 준 돈)")
                    
                    선급금_df['선급금'] = 선급금_df['미지급금'].abs()
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        st.metric("총 선급금", f"{선급금_df['선급금'].sum():,.0f}원")
                    with col2:
                        st.metric("선급금 거래처 수", f"{len(선급금_df)}개")
                    
                    display_선급금 = 선급금_df[['거래처', '선급금', '최근거래일']].copy()
                    display_선급금['최근거래일'] = pd.to_datetime(display_선급금['최근거래일']).dt.strftime('%Y-%m-%d')
                    display_선급금['선급금'] = display_선급금['선급금'].apply(lambda x: f"{x:,.0f}")
                    
                    st.dataframe(display_선급금, use_container_width=True, hide_index=True)
            else:
                st.info("매입 거래 내역이 없습니다.")
