...
# ==================== 거래처 관리 ====================
    ...
    def 구매주기_분석(거래처명, ledger_df):
        ...
        구매_df = 거래처_df[거래처_df['참조'].str.contains('외출', na=False)]  # 내가 판매한 것
        
        if len(구매_df) < 2:
            return []
        
        결과 = []
        품목_list = 구매_df['품목'].unique()
        
        for 품목 in 품목_list:
            품목_df = 구매_df[구매_df['품목'] == 품목].sort_values('날짜')
    ...
    def 판매기대치_계산(거래처명, ledger_df):
        """거래처의 판매 기대치 점수 계산"""
        거래처_df = ledger_df[ledger_df['거래처'] == 거래처명]
        판매_df = 거래처_df[거래처_df['참조'].str.contains('외출', na=False)]
    ...
    def 거래처_분류(ledger_df):
        ...
        for _, row in ledger_df.iterrows():
            ...
            if '외입' in 참조:  # 매입
                if 거래처 not in 매입거래처 or 날짜 > 매입거래처[거래처]:
                    매입거래처[거래처] = 날짜
            elif '외출' in 참조:  # 판매
                if 거래처 not in 판매거래처 or 날짜 > 판매거래처[거래처]:
                    판매거래처[거래처] = 날짜
        
        # 3개월 기준으로 활성/휴면 분류
        기준일 = datetime.now() - timedelta(days=90)
        
        매입_활성 = [(k, v) for k, v in 매입거래처.items() if v >= 기준일]
        매입_휴면 = [(k, v) for k, v in 매입거래처.items() if v < 기준일]
        판매_활성 = [(k, v) for k, v in 판매거래처.items() if v >= 기준일]
        판매_휴면 = [(k, v) for k, v in 판매거래처.items() if v < 기준일]
        
        # 최근 거래일 기준 정렬 (최신순)
        매입_활성.sort(key=lambda x: x[1], reverse=True)
        매입_휴면.sort(key=lambda x: x[1], reverse=True)
        판매_활성.sort(key=lambda x: x[1], reverse=True)
        판매_휴면.sort(key=lambda x: x[1], reverse=True)
    ...
    # 거래처 분류 실행
    분류된_거래처 = 거래처_분류(ledger_df) if len(ledger_df) > 0 else {'매입_활성': [], '매입_휴면': [], '판매_활성': [], '판매_휴면': []}
    
    # ========== 탭 구성 (4개 탭) ==========
    tab1, tab2, tab3, tab4 = st.tabs(["🎯 오늘의 영업", "📤 판매거래처", "📥 매입거래처", "➕ 거래처 추가"])
    ...
    with tab1:
        ...
        # 판매거래처만 대상으로 (내 고객!)
        판매_거래처_list = [x[0] for x in 분류된_거래처['판매_활성']]
        
        if len(판매_거래처_list) > 0:
            # 모든 거래처의 판매 기대치 계산
            거래처_기대치 = []
            for 거래처 in 판매_거래처_list:
    ...
    # ===== 탭2: 판매거래처 (내 고객) =====
    with tab2:
        st.markdown("### 📤 판매거래처 (내 고객)")
        st.markdown("*내가 물건을 판매하는 거래처*")
        
        판매_활성 = 분류된_거래처['판매_활성']
        판매_휴면 = 분류된_거래처['판매_휴면']
        
        if len(판매_활성) > 0 or len(판매_휴면) > 0:
            col1, col2 = st.columns([1, 2.5])
            
            with col1:
                # 활성 거래처 (3개월 내)
                if 판매_활성:
                    st.markdown(f"#### 🟢 활성 거래처 ({len(판매_활성)})")
                    판매_활성_names = [f"{x[0]} ({x[1].strftime('%m/%d')})" for x in 판매_활성]
                    선택_판매_활성 = st.radio("", 판매_활성_names, key="판매_활성", label_visibility="collapsed")
                    선택_거래처 = 선택_판매_활성.split(" (")[0] if 선택_판매_활성 else None
                else:
                    선택_거래처 = None
                
                # 휴면 거래처 (3개월 이상)
                if 판매_휴면:
                    with st.expander(f"⚪ 휴면 거래처 ({len(판매_휴면)}) - 3개월 이상 거래 없음"):
                        판매_휴면_names = [f"{x[0]} ({x[1].strftime('%m/%d')})" for x in 판매_휴면]
                        선택_판매_휴면 = st.radio("", 판매_휴면_names, key="판매_휴면", label_visibility="collapsed")
                        if 선택_판매_휴면:
                            선택_거래처 = 선택_판매_휴면.split(" (")[0]
            
            with col2:
                if 선택_거래처:
                    st.markdown(f"### 🏢 {선택_거래처}")
                    
                    # 거래처 거래 데이터
                    거래처_df = ledger_df[ledger_df['거래처'] == 선택_거래처]
                    
                    # ===== 미수금 현황 =====
                    st.markdown("#### 💰 미수금 현황")
                    
                    기초미수금_dict = base_recv_df.set_index('거래처')['기초미수금'].to_dict() if len(base_recv_df) > 0 else {}
                    기초미수금 = 기초미수금_dict.get(선택_거래처, 0)
                    ...
                    with col_a:
                        st.metric("기초미수금", f"{기초미수금:,.0f}원")
                    with col_b:
                        st.metric("총 외상", f"{총외상:,.0f}원")
                    with col_c:
                        st.metric("총 입금", f"{총입금:,.0f}원")
                    with col_d:
                    ...
                    # ===== 구매 패턴 분석 =====
                    st.markdown("#### 📦 구매 패턴 분석 (이 거래처가 구매하는 주기)")
                    
                    주기_분석 = 구매주기_분석(선택_거래처, ledger_df)
                    
                    if 주기_분석:
                        for item in 주기_분석[:10]:  # 상위 10개
                            남은일 = item['남은일수']
                            if 남은일 <= 0:
                                상태 = "🔴 구매일 지남!"
                                색상 = "#ffebee"
                            ...
                            st.markdown(f"""
                            <div style='background-color: {색상}; border-radius: 8px; padding: 10px; margin: 5px 0;'>
                                <b>{item['품목']}</b><br>
                                주기: {item['평균주기']}일 | 마지막: {item['마지막구매'].strftime('%m/%d')} | 
                                예상: {item['다음예상'].strftime('%m/%d')} | <b>{상태}</b>
                            </div>
                            """, unsafe_allow_html=True)
                    else:
                        st.info("구매 패턴을 분석하려면 2회 이상의 거래가 필요합니다.")
                    ...
                    if len(거래처_df) > 0:
                        # 당월 거래
                        현재월 = datetime.now().month
                        당월_df = 거래처_df[거래처_df['날짜'].dt.month == 현재월]
                        당월_금액 = 당월_df['공급가액'].sum() + 당월_df['부가세'].sum()
                        
                        # 총 거래
                        총_금액 = 거래처_df['공급가액'].sum() + 거래처_df['부가세'].sum()
                        
                        col_a, col_b, col_c = st.columns(3)
                        with col_a:
                            st.metric("총 거래 횟수", f"{len(거래처_df)}건")
                        with col_b:
                            st.metric("당월 거래", f"{len(당월_df)}건 / {당월_금액:,.0f}원")
                        with col_c:
                            st.metric("총 거래금액", f"{총_금액:,.0f}원")
                    ...
                    if len(거래처_df) > 0:
                        최근_거래 = 거래처_df.sort_values('날짜', ascending=False).head(15)
                        
                        display_df = 최근_거래[['날짜', '품목', '참조', '수량', '단가', '공급가액']].copy()
                        display_df['날짜'] = display_df['날짜'].dt.strftime('%Y-%m-%d')
                        display_df['수량'] = display_df['수량'].apply(lambda x: f"{x:,.0f}")
            ...
            st.info("판매거래처 데이터가 없습니다.")
    
    # ===== 탭3: 매입거래처 (공급처) =====
    with tab3:
        st.markdown("### 📥 매입거래처 (공급처)")
        st.markdown("*내가 물건을 구입하는 거래처*")
        
        매입_활성 = 분류된_거래처['매입_활성']
        매입_휴면 = 분류된_거래처['매입_휴면']
        
        if len(매입_활성) > 0 or len(매입_휴면) > 0:
            col1, col2 = st.columns([1, 2.5])
            
            with col1:
                # 활성 거래처 (3개월 내)
                if 매입_활성:
                    st.markdown(f"#### 🟢 활성 거래처 ({len(매입_활성)})")
                    매입_활성_names = [f"{x[0]} ({x[1].strftime('%m/%d')})" for x in 매입_활성]
                    선택_매입_활성 = st.radio("", 매입_활성_names, key="매입_활성", label_visibility="collapsed")
                    선택_매입거래처 = 선택_매입_활성.split(" (")[0] if 선택_매입_활성 else None
                else:
                    선택_매입거래처 = None
                
                # 휴면 거래처 (3개월 이상)
                if 매입_휴면:
                    with st.expander(f"⚪ 휴면 거래처 ({len(매입_휴면)}) - 3개월 이상 거래 없음"):
                        매입_휴면_names = [f"{x[0]} ({x[1].strftime('%m/%d')})" for x in 매입_휴면]
                        선택_매입_휴면 = st.radio("", 매입_휴면_names, key="매입_휴면", label_visibility="collapsed")
                        if 선택_매입_휴면:
                            선택_매입거래처 = 선택_매입_휴면.split(" (")[0]
            
            with col2:
                if 선택_매입거래처:
                    st.markdown(f"### 🏭 {선택_매입거래처}")
                    
                    # 거래처 거래 데이터 (매입만)
                    거래처_df = ledger_df[(ledger_df['거래처'] == 선택_매입거래처) & (ledger_df['참조'].str.contains('외입', na=False))]
                    ...
                    if len(거래처_df) > 0:
                        ...
                        col_a, col_b, col_c = st.columns(3)
                        with col_a:
                            st.metric("총 매입 횟수", f"{len(거래처_df)}건")
                        with col_b:
                        ...
                        for _, row in 품목별_매입.iterrows():
                            품목명 = row['품목'][:40] + '...' if len(row['품목']) > 40 else row['품목']
                            st.markdown(f"- **{품목명}**: {row['수량']:,.0f}개 / 평균 {row['단가']:,.0f}원")
                        
                        st.markdown("---")
                        
                        # ===== 최근 매입 내역 =====
                        st.markdown("#### 📅 최근 매입 내역")
                        
                        최근_거래 = 거래처_df.sort_values('날짜', ascending=False).head(15)
                        
                        display_df = 최근_거래[['날짜', '품목', '수량', '단가', '공급가액']].copy()
                        display_df['날짜'] = display_df['날짜'].dt.strftime('%Y-%m-%d')
                        display_df['수량'] = display_df['수량'].apply(lambda x: f"{x:,.0f}")
                        display_df['단가'] = display_df['단가'].apply(lambda x: f"{x:,.0f}")
                        display_df['공급가액'] = display_df['공급가액'].apply(lambda x: f"{x:,.0f}")
