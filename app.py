...
# ==================== 거래처 관리 ====================
    ...
    with tab2:
        ...
        if len(고객_활성) > 0 or len(고객_휴면) > 0:
            col1, col2 = st.columns([1, 2.5])
            
            with col1:
                # 활성 고객 (3개월 내)
                if 고객_활성:
                    st.markdown(f"#### 🟢 활성 고객 ({len(고객_활성)})")
                    고객_활성_names = [f"{x[0]} ({x[1].strftime('%m/%d')})" for x in 고객_활성]
                    선택_고객_활성 = st.radio("", 고객_활성_names, key="고객_활성", label_visibility="collapsed")
                    선택_고객 = 선택_고객_활성.split(" (")[0] if 선택_고객_활성 else None
                else:
                    선택_고객 = None
                
                # 휴면 고객 (3개월 이상)
                if 고객_휴면:
                    with st.expander(f"⚪ 휴면 고객 ({len(고객_휴면)}) - 3개월 이상 거래 없음"):
                        고객_휴면_names = [f"{x[0]} ({x[1].strftime('%m/%d')})" for x in 고객_휴면]
                        선택_고객_휴면 = st.radio("", 고객_휴면_names, key="고객_휴면", label_visibility="collapsed")
                        if 선택_고객_휴면:
                            선택_고객 = 선택_고객_휴면.split(" (")[0]
            
            with col2:
                if 선택_고객:
                    st.markdown(f"### 🏢 {선택_고객}")
                    
                    # 이 고객에게 판매한 거래 데이터 (외출만)
                    고객_df = ledger_df[(ledger_df['거래처'] == 선택_고객) & (ledger_df['참조'].str.contains('외출', na=False))]
                    ...
                    if len(고객_df) > 0:
                        ...
                        if len(최근60일_df) > 0:
                            ...
                            for _, row in 최근60일_df.iterrows():
                                ...
                                수량 = row['수량'] if pd.notna(row['수량']) else 0
                                단가 = row['단가'] if pd.notna(row['단가']) else 0
                                날짜 = row['날짜'].strftime('%m/%d')
                                
                                st.markdown(f"• **{품목명}**: {수량:,.0f}개 / 평균 {단가:,.0f}원 ({날짜})")
                        else:
                    ...
                    # ===== 구매 패턴 분석 =====
                    st.markdown("#### 📊 구매 패턴 분석 (이 고객이 구매하는 주기)")
                    
                    주기_분석 = 구매주기_분석(선택_고객, ledger_df)
                    
                    if 주기_분석:
                        for item in 주기_분석[:5]:  # 상위 5개
                            남은일 = item['남은일수']
                            if 남은일 <= 0:
                                상태 = "🔴 구매일 지남!"
                                색상 = "#ffebee"
            ...
            with col2:
                if 선택_매입업체:
                    ...
                    if len(매입_df) > 0:
                        ...
                        # 총 매입
                        총_금액 = 매입_df['공급가액'].sum() + 매입_df['부가세'].sum()
                        ...
                        for _, row in 품목별_매입.iterrows():
                            품목명 = row['품목'][:40] + '...' if len(str(row['품목'])) > 40 else row['품목']
